...
它是 Key Security 模块中的控制面 Skill，重点在于解释密钥调用、授权边界和使用模式，而不是直接认定密钥已经泄漏。
...
它更关注密钥控制面与调用模式，而不是明文密钥暴露。

### 6. 判断边界

- 不直接推断密钥泄露，需要更多访问上下文。
- 需要结合资产角色和调用背景解释风险。
- 不能因为单次异常调用就直接下“密钥已泄露”结论。
- 不能替代完整的云审计或取证分析。
...
- 当前它和 `private_key_exposure` 同属 Key Security 域，但分别覆盖控制面与明文暴露两个不同问题面。
