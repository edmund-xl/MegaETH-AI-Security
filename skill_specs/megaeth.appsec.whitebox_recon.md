...
它是 Whitebox 链路的起点 Skill，负责把“可疑线索”与“已确认问题”明确分开，为后续验证阶段提供清晰上游输入。
...
- 需要进入验证阶段的关键路径
- 针对验证阶段的优先检查建议
- 不带 confirmed 口径的候选问题分层

### 5. 触发与路由

当输入被归类为白盒侦察材料，且核心信息仍处于“待验证”状态时命中本 Skill。它不负责生成最终交付报告，而是为验证和综合报告提供上游结论。

如果输入已经以复现、验证或证据确认为主，则应路由到 `megaeth.appsec.whitebox_exploit_validation`，而不是继续停留在本 Skill。
