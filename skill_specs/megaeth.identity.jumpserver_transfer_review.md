...
它是 JumpServer 单文件链中的“文件落地与后续执行前链路” Skill，重点是帮助系统理解传输行为如何与后续命令或会话行为关联。
...
在多文件同批次场景下，它应先独立产出传输事实，再交给综合 Skill 统一解释。

### 6. 判断边界

- 文件传输本身不等于恶意。
- 需要结合命令、时间窗和执行链判断风险。
- 不能只因为文件名敏感就直接确认为高危攻击。
- 不能把传输行为直接当成已执行恶意命令。
