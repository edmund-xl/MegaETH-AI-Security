...
它是当前主线中典型的多文件综合 Skill，负责在单文件分析之上完成跨源串联和高层综合判断。
...
- 必须以单文件分析和事实链为基础，不能重新发明上游事实。
- Gemini 如参与增强，只能增强综合结论和专业判断段。
...
- 当前实现依赖单文件 JumpServer Skills 先产出分层事实，再由本 Skill 完成综合收口。
- 当前页面版和下载版应共享同一模板骨架。
